...
const exportCsv = (data)=>{
    ...
    const csvContent = "DATA;LINHA;ORIGEM;DESTINO;HOAR INICIO;HORA FIM;PASSAGEIRO;KM INICIO;KM FIM; TOTAL KM; CARRO;MOTORISTA\n"
        + data.map(dailyPart=>{
        return dailyPart.travels.filter(travel=>travel.line!="DESLOCAMENTO OCIOSO").map(travel=>{
            ...
            return `${date};${travel.line};${travel.origin};${travel.destiny};${startTime};${endTime};${travel.passenger};${travel.startKM};${travel.endKM};${travel.endKM-travel.startKM};${dailyPart.car.number};${dailyPart.driver.name}\n`
        }).join("")
    }).join("")
